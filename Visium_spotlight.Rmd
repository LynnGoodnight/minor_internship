...
#######################################################
# plot mitochondria gene number, n_genes", "nCount_RNA"
...
sc_dataset_filtered
VlnPlot(sc_dataset, features = c("n_genes", "nCount_RNA","percent.mt"))
```
...
```{r plot selected cell types on spatial images with one cell type per plot and contain concentration scale bar, warning=FALSE, fig.width=10, fig.height=10}
...
#  unloadNamespace("SPOTlight")
# library(SPOTlight, lib.loc="/data/home/bioinfo/R/R4.1.0/library_B3.13/old_versions/")
# for (selected_type in c( "B_GC_LZ", "B_GC_prePB","T_CD4+_TfH_GC","B_GC_DZ",
#                  "B_Cycling",  "FDC") )
# {
...
```{r calculate RMSE when deleted different cell types and plot 6 cell types}
...
spatial@meta.data[spatial@meta.data$GC == '',"GC"] = "Non germline center"
spatial@meta.data[spatial@meta.data$GC == 'GC',"GC"] = "Germline center"
                 ...
                 "B.Cycling",  "FDC") )
{
  ...
  if(selected_type != "T.CD4..TfH.GC"){
   selected_type_name = gsub(".", "_", selected_type,fixed = TRUE)
  }else{
    selected_type_name = "T_CD4+_TfH_GC"
  }

  spotlight_result_file = paste(dataset_name,"spotlight_deleted", selected_type_name, ".rds",sep ="_")
  setwd("/data/home/lyang/Visium_spotlight")
...
for (deleted_type in c("B.GC.DZ", "B.GC.LZ", "B.GC.prePB","T.CD4..TfH.GC", "B.Cycling",  "FDC","complete") )
{
  ...
  if(deleted_type == "complete"){
    rds_file= "old_version_ls.rds"
  }else if(deleted_type != "T.CD4..TfH.GC"){
   deleted_type_name = gsub(".", "_", deleted_type,fixed = TRUE)
  spotlight_result_file = paste(dataset_name,"spotlight_deleted", deleted_type_name, ".rds",sep ="_")
  rds_file= spotlight_result_file
  }else{
    deleted_type_name = "T_CD4+_TfH_GC"
    spotlight_result_file = paste(dataset_name,"spotlight_deleted", deleted_type_name, ".rds",sep ="_")
  rds_file= spotlight_result_file
  }
    spotlight_ls <- readRDS(rds_file)
